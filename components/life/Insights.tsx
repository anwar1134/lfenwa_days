...
import { C, Panel, Segmented, StatTile, MiniLineChart } from "./ui";
import { dbGetAll, readTradesKV } from "@/lib/storage";
import type { DayRecord, MoneyEntry, Habit, HabitEntry, TaskEntry, TradeLike } from "@/types/life";
...
function avg(arr: (number | null | undefined)[]): number | null {
  const nums = arr.filter((n): n is number => typeof n === "number" && !isNaN(n));
...
export default function Insights() {
  ...
  const reload = useCallback(async () => {
    ...
    setData({ days, money, habits: habits.filter((h) => !h.archived), habitEntries, tasks, trades: (trades as TradeLike[] | null) || [] });
  }, []);
  useEffect(() => {
    reload();
  }, [reload]);
  ...
  const inRange = (d: string) => d >= cutoff;
  ...
  const sleepAvg = avg(daysInRange.map((d) => (typeof d.sleepHours === "number" ? d.sleepHours : null)));
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
        ...
        <Segmented value={range} onChange={(v) => setRange(v as RangeKey)} options={[{ value: "week", label: "Week" }, { value: "month", label: "Month" }, { value: "year", label: "Year" }]} />
      </div>
