...
import { C, Panel, Field, TextInput, NumberInput, SelectInput, Segmented, StatTile, primaryBtn, fmtDateShort } from "./ui";
import { dbGetAll, dbGet, dbPut, dbDelete, uid, todayStr } from "@/lib/storage";
import type { MoneyEntry, MoneyType } from "@/types/life";

const CATEGORIES = ["Food", "Transport", "Housing", "Shopping", "Health", "Entertainment", "Bills", "Income", "Other"];

export default function Money() {
  const [items, setItems] = useState<MoneyEntry[]>([]);
  ...
  const sum = (arr: MoneyEntry[], type: MoneyType) => arr.filter((i) => i.type === type).reduce((s, i) => s + Number(i.amount || 0), 0);
...
function MoneyForm({ defaultCurrency, onDone }: { defaultCurrency: string; onDone: () => void }) {
