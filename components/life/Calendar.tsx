...
import { C, Panel, fmtDateLong, primaryBtn, ghostBtn } from "./ui";
import { dbGetAll, dbGet, getTradesForDate, todayStr, readTradesKV } from "@/lib/storage";
import type { DayRecord, TradeLike, StoreName } from "@/types/life";
...
const MARK_STORES: StoreName[] = ["days", "timeline", "achievements", "tasks", "learning", "money", "memories", "mindEntries"];

export default function Calendar({ onOpenDay }: { onOpenDay: (date: string) => void }) {
  ...
  const loadMonthData = useCallback(async () => {
    setLoading(true);
    const sets = await Promise.all(MARK_STORES.map((s) => dbGetAll(s)));
    const marks = new Set<string>();
    sets.forEach((rows) => rows.forEach((r) => "date" in r && r.date && marks.add(r.date)));
...
function DaySummaryPopup({ date, onClose, onOpenDay }: { date: string; onClose: () => void; onOpenDay: (date: string) => void }) {
  const [data, setData] = useState<{ day: DayRecord | Record<string, never>; trades: TradeLike[] } | null>(null);
  useEffect(() => {
    (async () => {
      ...
      <div onClick={(e) => e.stopPropagation()} style={{ background: C.panel, border: `1px solid ${C.line}`, borderRadius: 14, padding: 18, width: "100%", maxWidth: 380 }}>
        ...
        {!data ? (
          ...
          <div style={{ fontSize: 13, color: C.ink, lineHeight: 1.9 }}>
            ...
            <div>Sleep: {(data.day as DayRecord).sleepHours ?? "—"}h</div>
