...
import { C, Panel, TextInput, primaryBtn, StatTile } from "./ui";
import { dbGetAll, dbPut, dbGetByHabit, uid, todayStr } from "@/lib/storage";
...
export default function HealthHabits() {
  ...
  const reload = useCallback(async () => {
    ...
    if (hs.length === 0) {
      ...
      const seed: Habit[] = ["Exercise", "Study", "Drink water", "Sleep early"].map((n) => ({ id: uid(), name: n, createdAt: Date.now(), archived: false }));
  ...
  function completion(habit: Habit, days: number): number {
    const entries = entriesByHabit[habit.id] || [];
    const doneDates = new Set(entries.filter((e) => e.done).map((e) => e.date));
