...
import { C, Panel, Field, TextInput, TextArea, NumberInput, SelectInput, Segmented, RatingScale, primaryBtn, ghostBtn, fmtDateLong } from "./ui";
import { dbGet, dbPut, dbGetByDate, dbDelete, uid, todayStr, getTradesForDate } from "@/lib/storage";
import type { DayRecord, DayMetrics, DayReview, TimelineEntry, TaskEntry, Achievement, LearningEntry, TradeLike } from "@/types/life";

const SUBTABS = ["Basic", "Metrics", "Timeline", "Tasks", "Achievements", "Learning", "Review"] as const;
...
export default function MyDay({ date, setDate }: { date: string; setDate: (d: string) => void }) {
  const [sub, setSub] = useState<SubTab>("Basic");
  const [day, setDay] = useState<DayRecord>({ date });
  ...
  async function saveDay(patch: Partial<DayRecord>) {
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 14, overflowX: "auto" }}>
        <Segmented value={sub} onChange={(v) => setSub(v as SubTab)} options={[...SUBTABS]} />
      </div>
      ...
      {loading ? (
        ...
        <>
          ...
          {sub === "Achievements" && <AchievementsPanel date={date} items={achievements} reload={reload} />}
          {sub === "Learning" && <LearningPanel date={date} items={learning} reload={reload} />}
...
function BasicInfo({ day, onSave, trades }: { day: DayRecord; onSave: (p: Partial<DayRecord>) => void; trades: TradeLike[] }) {
  return (
    <div>
      {trades.length > 0 && (
...
function Metrics({ day, onSave }: { day: DayRecord; onSave: (p: Partial<DayRecord>) => void }) {
  const m = day.metrics || {};
  function set(key: keyof DayMetrics, v: number) {
...
function Timeline({ date, items, reload }: { date: string; items: TimelineEntry[]; reload: () => void }) {
...
function TimelineForm({ date, onDone }: { date: string; onDone: () => void }) {
...
function AchievementsPanel({ date, items, reload }: { date: string; items: Achievement[]; reload: () => void }) {
...
function Tasks({ date, items, reload }: { date: string; items: TaskEntry[]; reload: () => void }) {
  const [text, setText] = useState("");
  async function add() {
    if (!text.trim()) return;
  ...
  async function setStatus(t: TaskEntry, status: TaskEntry["status"]) {
      ...
      {items.map((t) => (
        <div key={t.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "7px 0", borderBottom: `1px solid ${C.lineSoft}` }}>
          ...
          <div style={{ display: "flex", gap: 4 }}>
            {(["pending", "done", "postponed"] as const).map((s) => (
              <button
                key={s}
...
const EMPTY_LEARNING_FORM: LearningFormState = { whatLearned: "", whatUnderstood: "", whatConfused: "", importantIdea: "", resource: "" };

function LearningPanel({ date, items, reload }: { date: string; items: LearningEntry[]; reload: () => void }) {
...
function Review({ day, onSave }: { day: DayRecord; onSave: (p: Partial<DayRecord>) => void }) {
  const r = day.review || {};
  function set(key: keyof DayReview, v: string | number) {
