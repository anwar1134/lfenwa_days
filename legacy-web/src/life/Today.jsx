...
import { C, Panel, StatTile, fmtDateLong, primaryBtn } from "./ui.jsx";
import { dbGetByDate, dbGet, getTradesForDate, todayStr } from "./storage.js";

export default function Today({ onNavigate, onQuickAdd }) {
  const date = todayStr();
      ...
      <Panel title="How today is going" right={<button onClick={() => onNavigate("myday")} style={{ background: "none", border: "none", color: C.accent, fontSize: 12, cursor: "pointer" }}>Open My Day →</button>}>
        {metrics.mood == null ? (
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            ...
            <StatTile label="Sleep" value={day?.sleepHours != null ? `${day.sleepHours}h` : "—"} />
          </div>
        )}
      </Panel>
