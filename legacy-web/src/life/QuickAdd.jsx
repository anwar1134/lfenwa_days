...
import { C, Field, TextInput, TextArea, NumberInput, SelectInput, RatingScale, Segmented, primaryBtn, ghostBtn } from "./ui.jsx";
import { dbPut, dbGet, dbGetAll, uid, todayStr, compressImage } from "./storage.js";
...
export default function QuickAdd({ onClose, onNavigate, defaultType }) {
...
function SaveRow({ onSave, disabled }) {
  return (
...
function DailyForm({ onDone }) {
  const date = todayStr();
  const [m, setM] = useState({ mood: null, energy: null, focus: null, motivation: null, stress: null, productivity: null });
...
function MemoryForm({ onDone }) {
  ...
  return (
    <div>
      <Field label="Type">
        <Segmented value={kind} onChange={setKind} options={[{ value: "text", label: "Text" }, { value: "photo", label: "Photo" }, { value: "file", label: "File" }]} />
      </Field>
...
function GoalProgressForm({ onDone }) {
