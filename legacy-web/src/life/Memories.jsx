...
import { FiPlus, FiTrash2, FiCamera, FiVideo, FiMic, FiFile, FiType, FiX, FiSquare } from "react-icons/fi";
import { C, Panel, Field, TextArea, Segmented, primaryBtn, ghostBtn, fmtDateLong } from "./ui.jsx";
import { dbGetAll, dbGet, dbPut, dbDelete, uid, todayStr, compressImage } from "./storage.js";
...
export default function Memories() {
  ...
  return (
    <div>
      ...
      {viewing && <MemoryViewer memory={viewing} attachment={viewing.attachmentId ? attachments[viewing.attachmentId] : null} onClose={() => setViewing(null)} onDeleted={() => { setViewing(null); reload(); }} />}
    </div>
  );
}

function MemoryViewer({ memory, attachment, onClose, onDeleted }) {
  return (
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
      <div onClick={(e) => e.stopPropagation()} style={{ background: C.panel, border: `1px solid ${C.line}`, borderRadius: 14, padding: 16, width: "100%", maxWidth: 420, maxHeight: "85vh", overflowY: "auto" }}>
        ...
        {memory.type === "photo" && attachment && <img src={attachment.dataUrl} alt="" style={{ width: "100%", borderRadius: 8, marginBottom: 10 }} />}
...
function MemoryForm({ onDone }) {
  ...
  async function startRecording() {
    ...
    try {
      ...
      setError("Microphone unavailable: " + (e.message || "permission denied"));
    }
  }
  ...
  async function save() {
    ...
    try {
      ...
      setError(e.message || "Couldn't save this memory.");
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <div>
      <Field label="Type">
        <Segmented
          value={kind}
          onChange={(v) => { setKind(v); setFile(null); setRecordedBlob(null); setError(""); }}
          options={[
