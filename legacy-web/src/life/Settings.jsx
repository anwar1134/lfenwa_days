...
import { C, Panel, Field, TextInput, primaryBtn, ghostBtn, dangerBtn } from "./ui.jsx";
import { dbGet, dbPut, exportFullBackup, importLifeBackup, downloadFile } from "./storage.js";

export default function Settings({ onNavigate }) {
  const [currency, setCurrency] = useState("DH");
  const [status, setStatus] = useState("");
  const [pendingImport, setPendingImport] = useState(null); // {payload, mode}
  ...
  async function saveCurrency(v) {
    setCurrency(v);
    const existing = (await dbGet("settings", "app")) || { id: "app" };
